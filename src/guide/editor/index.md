...
- `项目名称` 任意的项目名称。
- `项目类型` UI项目类型，即目标平台，当前支持有Unity、Haxe、Starling、Egret、Layabox、Flash。不同的平台类型在资源组织、发布上有一定的差别。不需要担心这里选择错了项目类型，在项目创建后可以随时调整UI项目类型，操作位置在菜单“文件”->“项目属性”里。
...
库面板里采用树状结构显示。顶层节点是包，每个包下面可以创建文件夹。
...
- `不生成使用默认名称的成员` 对于扩展组件，会有一些约定名称的元件，例如按钮里的“title”，“icon”等。这些成员已经提供了API访问，因此不需要再导出。勾选后，将忽略这些使用约定名称的元件。注意，如果一个组件里，只有约定名称的元件，而这个选项也勾选，那么整个组件都不会生成代码。例如，一个按钮，里面只有名称为“title”和“icon”的元件，则不会为这个按钮生成代码，因为使用基类GButton已经足够。
