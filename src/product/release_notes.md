...
## 3.9.9
增加了MonoGame引擎的支持，欢迎使用。

## 3.9.8
1. 设计参考图现在可以选择是否在测试视图也显示。
2. 修正了文本“自动缩小”功能在编辑器预览不正确的bug。
3. 修正了退出测试视图时，下拉框仍然显示的bug。
4. 修正了导出C++代码不正确的bug。
5. 
## 3.9.7
1. 增加了一个创建窗口框架的向导对话框（在资源菜单下）。
2. 修正了列表item编辑界面的一个bug。

## 3.9.6
1. 现在支持跨包移动资源。
2. 优化了列表item编辑界面。
3. 修正了字符串导入导出功能的一个bug（有关文本控制）。

## 3.9.5
1. 动效新增了"改变文字”和“改变图标”两种动画类型。
2. 修正了Mac系统下不能使用Command+C/X/V/A的问题。
3. 修正了相同前缀的包在发布时可能错误删除文件的问题。

## 3.9.3
1. 修正发布二进制格式时文字投影不正确的bug。

## 3.9.2
1. 增加对位图字体勾选了“允许动态字号”后，字体大小错误设置为0的情况的兼容。
...
3. 列表项目编辑界面增加了“发布时清楚”设置，勾选后，最终发布时不会包含这里定义的列表数据，也就是列表数据将仅作编辑器预览用途。
